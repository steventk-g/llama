...
def init(
    ...
    x_dim_att = math.isqrt(num_devices)
    yz_dim_att = math.isqrt(num_devices)


    x_dim_ff = math.isqrt(num_devices) // 2
    yz_dim_ff = 2 * math.isqrt(num_devices)

    col_mesh = xs.Mesh(device_ids, (1, num_devices))
    row_mesh = xs.Mesh(device_ids, (num_devices, 1))
    
    two_d_mesh_att = xs.Mesh(device_ids, (x_dim_att, yz_dim_att))
    two_d_mesh_transpose_att = xs.Mesh(device_ids, (yz_dim_att, x_dim_att))

    two_d_mesh_ff = xs.Mesh(device_ids, (x_dim_ff, yz_dim_ff))
    two_d_mesh_transpose_ff = xs.Mesh(device_ids, (yz_dim_ff, x_dim_ff))
    ...
    for name, layer in model.named_modules():
        ...
        if 'attention.' in name:
            if 'wo' in name:
                xs.mark_sharding(layer.weight, two_d_mesh_transpose_att, (0, 1))
            else:
                xs.mark_sharding(layer.weight, two_d_mesh_att, (0, 1))
        if 'feed_forward.' in name:
            if 'w2' in name:
                xs.mark_sharding(layer.weight, two_d_mesh_transpose_ff, (0, 1))
            else:
                xs.mark_sharding(layer.weight, two_d_mesh_ff, (0, 1))
        if 'output' in name:
